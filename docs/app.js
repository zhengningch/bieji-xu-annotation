...
let DB;
fetch('data.json').then(r=>r.json()).then(data=>{DB=data;setup();render()}).catch(e=>{$('#analysis').innerHTML='<div class=card style="color:#a00">資料載入失敗：'+e+'</div>'});
...
function selected(){let v=$('#version').value,p=$('#period').value,u=$('#author').value,g=$('#genre').value,q=$('#query').value.trim();return DB.articles.filter(a=>a.labels[v]&&(!p||a.period===p)&&(!u||a.author===u)&&(!g||a.genre===g)&&(!q||[a.title,a.author,...a.texts].join('').includes(q)))}
function labels(a){return a.labels[$('#version').value]}
